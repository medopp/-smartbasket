...
export async function handle(req,env){
 ...
 if(route.startsWith('/api/')){
  ...
  if(route==='/api/shipments'&&method==='POST'){
   ...
   let inheritedDate=b.date||null;
   if(trip!=='لم تُحدد'){const existing=await db('sb_shipments?trip=eq.'+encodeURIComponent(trip)+'&country=eq.'+encodeURIComponent(b.country)+'&mode=eq.'+encodeURIComponent(b.mode));if(existing.some(s=>Number(s.step)>0))fail(400,'لا يمكن إضافة شحنة إلى رحلة غادرت المخزن.');if(!inheritedDate)inheritedDate=existing.find(s=>s.ship_date)?.ship_date||null;}
   await db('sb_shipments','POST',{id,customer_code:customer,country:b.country,mode:b.mode,weight,unit,trip,ship_date:inheritedDate});return json({ok:true},201);
  }
