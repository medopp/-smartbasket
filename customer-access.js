(() => {
  ...
  function setLoginScreen() {
    ...
    if (!modal) return;
    modal.querySelector("h2").textContent = "دخول العميل";
    modal.querySelector("p").textContent = "اكتب كود العميل وكلمة المرور اللذين استلمتهما من الشركة.";
    document.getElementById("emailRegisterTab")?.parentElement?.remove();
    const fields = modal.querySelectorAll(".auth-field");
    if (fields[0]) fields[0].querySelector("label").textContent = "كود العميل";
    const code = document.getElementById("emailInput");
    if (code) { code.type = "text"; code.placeholder = "مثال: LN-1010"; code.autocomplete = "username"; }
  ...
  window.emailAuthContinue = async () => {
    const code = document.getElementById("emailInput").value.trim().toUpperCase();
    const password = document.getElementById("passwordInput").value;
    if (!code || !password) return window.setAuthStatus("اكتب كود العميل وكلمة المرور.", true);
    if (!/^[A-Z]{2,8}-\d{3,10}$/.test(code)) return window.setAuthStatus("صيغة الكود مثال: LN-1010", true);
    window.setAuthStatus("جاري تسجيل الدخول...");
    const { data, error } = await supabaseClient.auth.signInWithPassword({ email: emailForCode(code), password });
    if (error) return window.setAuthStatus("كود العميل أو كلمة المرور غير صحيحة.", true);
    window.closeAuth();
