...
const customerEmail = code =>
  "customer-" + code.toLowerCase().replace(/[^a-z0-9]/g, "") + "@accounts.smartbasket.local";
const adminEmails = new Set(["jejohar@gmail.com", "jejohaly@gmail.com"]);
...
export async function onRequest(context) {
  ...
  try {
    ...
    const isAdmin = meResponse.ok && (
      me.app_metadata?.role === "admin" ||
      adminEmails.has(String(me.email || "").toLowerCase())
    );
    if (!isAdmin) return reply({ error: "هذه الخدمة متاحة للمدير فقط" }, 403);
    ...
    const created = await fetch(baseUrl + "/auth/v1/admin/users", {
      ...
      body: JSON.stringify({
        email: customerEmail(code),
        password,
        email_confirm: true,
        app_metadata: { role: "customer", customer_code: code },
        user_metadata: {
    ...
    if (!created.ok) {
      return reply({ error: result?.msg || result?.message || "تعذر إنشاء حساب العميل" }, created.status);
    }
    return reply({ success: true, customerCode: code, customerId: result.id || result.user?.id });
  } catch (error) {
    return reply({ error: "حدث خطأ أثناء إنشاء العميل", details: error?.message || String(error) }, 500);
  }
}
