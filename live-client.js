...
function tab(name){['ship','ware','manage',...(window.SBOps?.tabs||[])].forEach(id=>{if(!$(id))return;$(id).hidden=id!==name;$(id+'-tab').classList.toggle('active',id===name)});window.SBOps?.opened(name)}
function filterShipments(){if(window.SBOps)return window.SBOps.filter(state);const q=$('search').value.trim().toUpperCase(),country=$('filter-country').value,mode=$('filter-mode').value;return state.shipments.filter(s=>s.id.includes(q)&&(!country||s.country===country)&&(!mode||s.mode===mode))}
...
function openCustomerTrip(group){
 ...
 if(window.SBOps?.requestDelivery&&group.rows.some(s=>s.step!==4)){const b=document.createElement('button');b.type='button';b.className='primary';b.textContent='طلب توصيل شحنات الرحلة للبيت';b.onclick=()=>window.SBOps.requestDelivery(group.rows.filter(s=>s.step!==4).map(s=>s.id));d.querySelector('.muted').after(b)}
...
function openEditShipment(s){
 ...
 window.SBOps?.enhanceForms();d.showModal();
}
...
function renderAddresses(){if(state.user.role==='customer')$('address-customer').innerHTML='';else{$('address-customer').innerHTML=`<label>عرض العنوان بكود الزبون <select id="address-code">${state.users.filter(c=>c.role==='customer'&&c.active).map(c=>`<option value="${esc(c.code)}">${esc(c.code)} · ${esc(c.name)}</option>`).join('')}</select></label>`;if($('address-code'))$('address-code').onchange=e=>{currentCustomer.code=e.target.value;renderWarehouses()}}if(currentCustomer.code)renderWarehouses();else $('warehouses').innerHTML='<article>أضف زبونًا لعرض العنوان بكوده.</article>'}
 async function refresh(){state=await api('/state');$('auth').hidden=true;$('workspace').hidden=false;$('identity').textContent=state.user.name+' · '+state.user.code;$('greeting').textContent=state.user.role==='customer'?'شحناتك، خطوة بخطوة':state.user.role==='staff'?'مساحة الموظف':'لوحة الإدارة';$('manage-tab').hidden=['customer','accountant'].includes(state.user.role);currentCustomer.code=state.user.role==='customer'?state.user.code:state.users.find(c=>c.role==='customer'&&c.active)?.code||'';window.SBOps?.sync();renderShipments();renderForms();renderAddresses();window.SBOps?.enhanceForms()}
...
if($('auth-form'))$('auth-form').onsubmit=async e=>{e.preventDefault();$('auth-submit').disabled=true;try{const data=Object.fromEntries(new FormData(e.target));if(setup){await api('/setup','POST',data);setup=false;$('setup-key').hidden=true;$('setup-name').hidden=true;$('setup-name').querySelector('input').required=false;$('auth-title').textContent='تسجيل الدخول';$('auth-submit').textContent='دخول'}await api('/login','POST',data);e.target.reset();$('auth-message').textContent='';await refresh();tab('ship')}catch(error){$('auth-message').textContent=error.message}finally{$('auth-submit').disabled=false}};
 ...
 import('/operations.mjs').then(mod=>{window.SBOps=mod;mod.init({getState:()=>state,api,refresh,esc,notice,tab,details,openEditShipment,renderShipments});if(state){mod.sync();renderShipments();mod.enhanceForms()}}).catch(()=>notice('تعذّر تحميل أدوات التشغيل. حدّث الصفحة وأعد المحاولة.'));
